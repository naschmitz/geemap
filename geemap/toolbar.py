...
def main_toolbar(m, position="topright", **kwargs):
    ...
    def layers_btn_click(change):
        if change["new"]:
            ...
            if m.layer_manager_control:
                return

            def _on_open_vis(layer_name):
                m.create_vis_widget(m.ee_layer_dict.get(layer_name, None))

            m.layer_manager_widget = map_widgets.LayerManager(m)
            m.layer_manager_widget.header_hidden = True
            m.layer_manager_widget.close_button_hidden = True
            m.layer_manager_widget.on_open_vis = _on_open_vis
            m.layer_manager_control = ipyleaflet.WidgetControl(
                widget=m.layer_manager_widget, position=position
            )
            toolbar_footer.children = [m.layer_manager_widget]
        else:
            toolbar_footer.children = [toolbar_grid]

            m.toolbar_reset()
            if m.layer_manager_control:
                if m.layer_manager_control in m.controls:
                    m.remove_control(m.layer_manager_control)
                m.layer_manager_control.close()
                m.layer_manager_control = None
